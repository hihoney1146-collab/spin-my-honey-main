...
/** Embed full sitemap body for the Vercel /api/sitemap handler (no CSV/fs at runtime). */
function writeApiSitemapPayload(root, xml) {
...
export function writeAllSitemapFiles(root = getProjectRoot()) {
  ...
  // Single canonical sitemap for GSC (flat urlset; legacy paths 301 here via vercel.json).
  fs.writeFileSync(path.join(publicDir, "sitemap.xml"), xml, "utf8");
  writeApiSitemapPayload(root, xml);

  for (const legacy of ["sitemap", "sitemap.txt", "sitemap-index.xml"]) {
