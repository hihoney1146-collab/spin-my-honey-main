...
export const CHILD_SITEMAPS = [
  {
    ...
    extensionless: "pages-sitemap",
    builder: buildPagesSitemapXml,
  },
  {
    filename: "wheels-sitemap.xml",
    extensionless: "wheels-sitemap",
    builder: buildWheelsSitemapXml,
  },
  {
    filename: "blog-sitemap.xml",
    extensionless: "blog-sitemap",
    builder: buildBlogSitemapXml,
  },
  {
    filename: "images-sitemap.xml",
    extensionless: "images-sitemap",
    builder: buildImagesSitemapXml,
  },
];
...
export function buildSitemapIndexXml(root = getProjectRoot()) {
  ...
  // Prefer extensionless locs — Cloudflare often blocks/mangles *.xml for Googlebot/GSC.
  const entries = CHILD_SITEMAPS.map((c) =>
    sitemapIndexEntry(`/${c.extensionless}`, lastmod),
  );
...
/** Embed full sitemap bodies for the Vercel /api/sitemap handler (no CSV/fs at runtime). */
function writeApiSitemapPayload(root, xml, txt) {
  const outPath = path.join(root, "api", "sitemap-payload.js");
  const body = `/**
 * AUTO-GENERATED by scripts/generate-sitemap.mjs — do not edit.
 * Full urlset so /api/sitemap works on Vercel without wheel CSV / generated JSON.
 */
export const SITEMAP_XML = ${JSON.stringify(xml)};
export const SITEMAP_TXT = ${JSON.stringify(txt)};
`;
...
export function writeAllSitemapFiles(root = getProjectRoot()) {
  ...
  // serverless rebuild (wheels/blogs missing when CSV isn't in the function bundle).
  fs.writeFileSync(path.join(publicDir, "sitemap"), xml, "utf8");
  ...
  writeApiSitemapPayload(root, xml, txt);
  ...
  for (const { filename, extensionless, builder } of CHILD_SITEMAPS) {
    const childXml = builder(root);
    fs.writeFileSync(path.join(publicDir, filename), childXml, "utf8");
    // Extensionless twins for GSC (*.xml often shows "Couldn't fetch" behind Cloudflare).
    fs.writeFileSync(path.join(publicDir, extensionless), childXml, "utf8");
  }
...
export function buildLlmsTxt(root = getProjectRoot()) {
  ...
  return [
    ...
    `- Child partitions: ${SITE}/pages-sitemap, ${SITE}/wheels-sitemap, ${SITE}/blog-sitemap, ${SITE}/images-sitemap`,
